...
std::string buffer;
serial::Serial ser;//self-defined message type


std::string num_int2hex(int numb){//translate hex number into int
    std::string output;
    switch (numb){
...
std::string int2hex(unsigned char num){//translate hex string to int number
    int number;
    int first_num;
    int second_num;
    std::string result;
...
int main (int argc, char** argv){
    ros::init(argc, argv, "serial_port_read0");
    ros::NodeHandle nh;

   // ros::Subscriber write_sub = nh.subscribe("write", 1000, write_callback);
    ros::Publisher msg_pub = nh.advertise<std_msgs::String>("Serial_read_0", 1000);

    try
    {
        ser.setPort("/dev/ttyUSB0");//set serial port CHANGE THE NAME TO THE AVALIABLE PORTS
        ser.setBaudrate(115200);//Serial port rate
    ...
    ros::Rate loop_rate(3000);//SPIN RATE depends on the refresh rate of data
    while(ros::ok()){
        std_msgs::String msg;
        ros::spinOnce();
        if(ser.available()){
            ...
            for(int l=0;l<rBUFFERSIZE;l++)
            {
                ...
                if(r_buffer[l] == 0xAA && char_buffer == 0x55)
                {
                    start_indicator = 0;
                    buffer = buffer + "AA";
                    msg.data = buffer;
