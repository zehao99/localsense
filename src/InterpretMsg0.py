...
tag0_position = String()
#position_record = list()
...
def msg_callback0(ros_str):
    ...
    #tag0_position.data = str(tag_id) + ' ' + str(curr_tag_position[0]) + ' ' + str(curr_tag_position[1]) + ' ' + str(curr_tag_position[2])
    #position_pub = rospy.Publisher("/tag_position",String,queue_size=1)
    #position_pub.publish(tag0_position)
    with open('tag0_record_16041.csv','a') as out:
        csv_write = csv.writer(out, dialect = 'excel')
        now = str(rospy.Time.now())
        #One station
        
        current_line = [
            now, tag_id, curr_tag_position[0], curr_tag_position[1], curr_tag_position[2], 0, tag_to_station['76124']
        ]
...
def main(args):
    ...
    msg_sub0 = rospy.Subscriber("/Serial_read_0",String, msg_callback0)
    try:
        rospy.spin()
